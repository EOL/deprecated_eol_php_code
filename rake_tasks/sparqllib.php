...
class sparql_connection
{
	...
	{	
		...
		$output = null; unset($output);
		$prefixes = null; unset($prefixes);
	...
	function alive( $timeout=3 )
	{
		...
		if( $this->errno ) { 
			debug("Query error: $this->error, error number: $this->errno");
			return false; 
		}
	...
	function dispatchQuery( $sparql, $timeout=null )
	{
		...
		if( $timeout !== null )
		{
			curl_setopt($context, CURLOPT_TIMEOUT, $timeout );
		}
		curl_setopt($context, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($context, CURLOPT_HTTPHEADER,array (
		...
		{
			$this->errno = curl_errno( $context );
			$this->error = 'Curl error: ' . curl_error($context);
			return;
		}
		if( $output === '' )
		{
