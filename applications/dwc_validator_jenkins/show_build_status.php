...
elseif($ctrler->is_build_currently_running($build_status)) {
    ...
    $path = "task_status.php?task=$task&uuid=$params[uuid]";
    ...
    if(true)
    {
        // print_r($params); exit("<br>elix<br>");
...
if($build_status) {
    // /* good original showing of build status. Commented here only in dwc_validator tool
    echo "<hr><b>Build status:</b><pre>".$build_status."</pre><hr>";
    // */
}
else
{
    if($ctrler->is_task_in_queue("dwca_validator_job", $params['uuid'].$postfix)) {
