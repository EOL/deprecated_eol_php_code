...
elseif($ctrler->is_build_currently_running($build_status)) {
    ...
    */
    ...
    $extensions = array('.tab', '.txt', '.tsv', '.zip', '.tmp'); //possible extensions
...
if($build_status) echo "<hr><b>Build status x:</b><pre>".$build_status."</pre><hr>";
else {
    if($ctrler->is_task_in_queue('xls2dwca_job', $params['uuid'].$postfix)) { //job_name = 'specmnXport_job' or 'xls2dwca_job'
        echo "<hr><b>Build status y:</b><pre>This job is now in queue...</pre><hr>";
    }
    else echo "<hr><b>Build status z:</b><pre>Preparing files...</pre><hr>";
}
if($build_status) {
