...
class SpeciesStats extends MysqlBase
{
    ...
    {   
        ...
        $pages_incol = $stats->total_pages_in_col();
        $pages_not_incol = $stats->total_pages_not_in_col();
        $taxa_count = $stats->pages_with_content();
        $taxa_text = $stats->pages_with_text();
        $taxa_images = $stats->pages_with_images();
        $taxa_text_images = $stats->pages_with_text_and_images();
        $taxa_images_no_text = $stats->pages_with_images_no_text();
        $taxa_text_no_images = $stats->pages_with_text_no_images();
        $taxa_links_no_text = $stats->pages_with_links_no_text();

        $vet_obj = $stats->pages_with_vetted_objects();
        $no_vet_obj2 = $stats->pages_in_col_no_content();
        $vet_obj_only_1cat_inCOL = $stats->pages_in_col_one_category();
        $vet_obj_only_1cat_notinCOL = $stats->pages_not_in_col_one_category();
        $vet_obj_morethan_1cat_inCOL = $stats->pages_in_col_more_categories();
        $vet_obj_morethan_1cat_notinCOL = $stats->pages_not_in_col_more_categories();

        $with_BHL           = $stats->pages_with_bhl();
        $taxa_BHL_no_text   = $stats->pages_with_bhl_no_text();
                
        //$vetted_not_published                           = $stats->pages_awaiting_publishing();
        ...
        $lifedesk_taxa          = $stats->lifedesk_taxa();
        $lifedesk_dataobject    = $stats->lifedesk_data_objects();
        
        $qry = " insert into page_stats_taxa(date_created,time_created,active,
        pages_incol,pages_not_incol,taxa_count,taxa_text,taxa_images,taxa_text_images,taxa_images_no_text,taxa_text_no_images,taxa_links_no_text,
        vet_obj,no_vet_obj2,vet_obj_only_1cat_inCOL,vet_obj_only_1cat_notinCOL,vet_obj_morethan_1cat_inCOL,vet_obj_morethan_1cat_notinCOL,
        with_BHL,taxa_BHL_no_text,
        vetted_not_published,vetted_unknown_published_visible_inCol,vetted_unknown_published_visible_notinCol,
        lifedesk_taxa,lifedesk_dataobject)
        select '$date_created','$time_created','n',
        $pages_incol,$pages_not_incol,$taxa_count,$taxa_text,$taxa_images,$taxa_text_images,$taxa_images_no_text,$taxa_text_no_images,$taxa_links_no_text,
        $vet_obj,$no_vet_obj2,$vet_obj_only_1cat_inCOL,$vet_obj_only_1cat_notinCOL,$vet_obj_morethan_1cat_inCOL,$vet_obj_morethan_1cat_notinCOL,
        $with_BHL,$taxa_BHL_no_text,
        $vetted_not_published,$vetted_unknown_published_visible_inCol,$vetted_unknown_published_visible_notinCol,
