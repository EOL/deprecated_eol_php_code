...
class Resource extends ActiveRecord
{
    ...
    public function harvest($validate = true, $validate_only_welformed = false, $fast_for_testing = false)
    {
        ...
        if($valid)
        {
            // TODO - this block should be cleaned up; there should be a
            // "prepare for harvest" or something like it.
            // TODO - this should be in a try/catch/finally, so that we abort
            // the transaction if anything fails:
            $this->mysqli->begin_transaction();
            $this->insert_hierarchy();
            $sparql_client = SparqlClient::connection();
            // TODO - It would be better if we could create a temporary graph
            // and rename it after we delete the old one. Better still if we had
            // a clever way to get diffs and just apply those to the existing
            // graph!
    ...
    {
        // TODO - we should do something more intelligent if this is NOT the
        // case; at the moment we simply ignore the harvest entirely if there
        // is already a harvest event attached.
        if(!$this->harvest_event)
        {
    ...
    // Creates a hierarchy for resources that don't have one yet.
    // TODO - evaluate whether ANY resources have a better description than the
    // default. It may be worth just removing that attribute.
    public function insert_hierarchy()
    {
        // Nothing to do if we already have an ID:
        ...
        // NOTE - I am not sure where complete gets set to true. :|
        $params["complete"] = 0;
