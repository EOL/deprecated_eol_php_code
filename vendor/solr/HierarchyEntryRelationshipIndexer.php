...
class HierarchyEntryRelationshipIndexer
{
    ...
    public function index($hierarchy, $compare_to_hierarchy = null)
    {
        ...
        {
            echo "(hierarchy_id_1:$hierarchy->id AND hierarchy_id_2:$compare_to_hierarchy->id) OR (hierarchy_id_2:$hierarchy->id AND hierarchy_id_1:$compare_to_hierarchy->id)\n";
        ...
        $max_id = 0;
        $limit = 100000;
        $result = $this->mysqli->query("SELECT MIN(hierarchy_entry_id_1) as min, MAX(hierarchy_entry_id_1) as max FROM he_relations_tmp he");
    ...
    private function lookup_relatipnships($start, $limit)
    {
        ...
        $outfile = $this->mysqli->select_into_outfile("SELECT he1.id id1, he1.taxon_concept_id taxon_concept_id1, he1.hierarchy_id hierarchy_id1, he1.visibility_id visibility_id1, he2.id id2, he2.taxon_concept_id taxon_concept_id2, he2.hierarchy_id hierarchy_id2, he2.visibility_id visibility_id2, he1.taxon_concept_id=he2.taxon_concept_id same_concept, hr.relationship, hr.score FROM he_relations_tmp hr JOIN hierarchy_entries he1 ON (hr.hierarchy_entry_id_1=he1.id) JOIN hierarchy_entries he2 ON (hr.hierarchy_entry_id_2=he2.id) WHERE he1.id BETWEEN $start AND ".($start+$limit));
