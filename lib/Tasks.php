...
class Tasks
{
    ...
    public static function update_taxon_concept_names($taxon_concept_id)
    {
        ...
        $result = $mysqli->query("(SELECT id, name_id, 'preferred' as type FROM hierarchy_entries WHERE taxon_concept_id=$taxon_concept_id) UNION (SELECT s.hierarchy_entry_id, s.name_id, 'synonym' as type FROM hierarchy_entries he JOIN synonyms s ON (he.id=s.hierarchy_entry_id) WHERE he.taxon_concept_id=$taxon_concept_id AND s.language_id=0 AND s.synonym_relation_id!=".SynonymRelation::find_or_create_by_label('genbank common name')->id." AND s.synonym_relation_id!=".SynonymRelation::find_or_create_by_label('common name')->id." AND s.synonym_relation_id!=".SynonymRelation::find_or_create_by_label('blast name')->id." AND s.synonym_relation_id!=".SynonymRelation::find_or_create_by_label('genbank acronym')->id." AND s.synonym_relation_id!=".SynonymRelation::find_or_create_by_label('acronym')->id.")");
        while($result && $row=$result->fetch_assoc())
        {
            $id = $row["id"];
            $name_id = $row["name_id"];
