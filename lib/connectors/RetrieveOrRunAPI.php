...
class RetrieveOrRunAPI
{
    function __construct($task2run, $download_options, $main_path)
    {
        ...
        $this->main_path = $this->download_options['cache_path'].$main_path."/";
    ...
    function retrieve_data($input, $filename)
    {   
        ...
        if(file_exists($filename)) {
            ...
            if($this->del_options['expire_seconds'] === false)             return self::retrieve_json($filename); //doesn't expire
