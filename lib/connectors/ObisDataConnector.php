...
class ObisDataConnector
{
    ...
    public function process_line_data($line_data)
    {
        ...
        {
            if(!$value) continue;
            if(!isset($this->column_headers[$index]))
            {
                echo "There is a problem - more columns in this row than in the header\n";
                continue;
            }
            $column_header = $this->column_headers[$index];
            if(in_array($column_header, $fields_to_ignore)) continue;
            $m = new \eol_schema\MeasurementOrFact();
            $m->measurementType = "http://iobis.org/". $column_header;
