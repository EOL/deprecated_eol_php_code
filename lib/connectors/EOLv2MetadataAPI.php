...
class EOLv2MetadataAPI
{
    ...
    public function start()
    {
        ...
        $sql = "SELECT cp.id as partner_id, cp.full_name as partner_name, s.label as status, r.id as resource_id, r.title as resource_title, 
            ...
            cp.description_of_data as desc_of_data
            FROM content_partners cp
            ...
            while($result && $row=$result->fetch_assoc()) {
                ...
                $recs[$row['partner_id']] = array('partner_name' => $row['partner_name'], 'partner_id' => $row['partner_id'], 
                'overview' => $row['overview'], 'url' => $row['url'], 'agreement_url' => $row['agreement_url'], 'signed_by' => $row['signed_by'], 'signed_date' => $row['signed_date'],
                'create_date' => $row['create_date'], 'desc_of_data' => $row['desc_of_data']
                );
                $recs[$row['partner_id']]['resources'][] = array('resource_title' => $row['resource_title'], 'first_pub' => $first_pub, 'last_pub' => $last_pub);
                
                $sql = "SELECT cpc.given_name, cpc.family_name, cpc.email, cpc.homepage, cpc.telephone, cpc.address, s.label as contact_role
                ...
                while($contacts && $row2=$contacts->fetch_assoc()) {
                    $recs[$row['partner_id']]['contacts'][] = array('given_name' => $row2['given_name'], 'family_name' => $row2['family_name'], 'email' => $row2['email'],
