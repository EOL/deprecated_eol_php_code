...
class FishWiseAPI
{
    ...
    {    
        ...
        "IsLegal",
        "Location","PicComments","IsAvailable","LifeStage",
        "CollectionName","CollectionAcronym",
        "PictureSource","Surname","Firstname","DisplayName",
        "FileName");        
        ...
        foreach($taxa_arr as $taxon_arr)
        {
            ...
            if($i % 10000 == 0)
            {
                $j++; $xml = SchemaDocument::get_taxon_xml($all_taxa);
                if($j < 10) $j_str = substr(strval($j/100),2,2);
                else        $j_str = strval($j);                    
                $resource_path = CONTENT_RESOURCE_LOCAL_PATH . "/FishWise/" . $j_str . ".xml";
    ...
    function combine_all_xmls($resource_id)
    {
        ...
        while(true)
        {
            ...
            print " $i "; 
            $filename = CONTENT_RESOURCE_LOCAL_PATH . "FishWise/" . $i_str . ".xml";
            if(!is_file($filename))
            {
                print" - not yet ready";
                break;
    ...
    function compile_all_xml($resource_id)
    {
        $old_resource_path = CONTENT_RESOURCE_LOCAL_PATH . $resource_id .".xml";
        $OUT = fopen($old_resource_path, "w+");
        $str = "<?xml version='1.0' encoding='utf-8' ?>\n";
        ...
        while(true)
        {
            ...
            if($str)
            {
                $pos1 = stripos($str,"<taxon>");
                $pos2 = stripos($str,"</response>");            
                if($pos1 != "" and $pos2 != "")
                {
                    $str = trim(substr($str,$pos1,$pos2-$pos1));
                    fwrite($OUT, $str);
                }                
            }            
            else break;    
    ...
    function compile_taxa($urls)
    {
        ...
        {            
            ...
            "Habitat","HabitatNotes",    
            "DepthRange","DepthRangeShallow","DepthRangeDeep",    
            "LengthMax","LengthMaxSuffix","LengthMaxType",
            "Journal","Citation","TextPage"            
            );                        
    ...
    function search_collections($taxon,$taxon_images,$taxon_comnames,$taxon_syn)//this will output the raw (but structured) output from the external service
    {
        $taxon_id = $taxon["SId"];                    
