...
/* connector: [741] IRMNG data
              [752] IRMNG classification
...
class IrmngAPI
{
    function __construct($folder)
    {
        ...
        $this->limit = 100000; // no. of batch process for MeasurementOfFact
        $this->TEMP_DIR = create_temp_dir() . "/";
        $this->temp_archive_builder = false;
        ...
        */
        ...
        $this->debug_status = array();
        $this->debug_status_count = array();
        
    }

    function get_all_taxa($resource_type)
    {
        ...
        {
            $count_of_taxa_ids_with_data = self::process_data_classification();
            $i = 0;
            $total = $count_of_taxa_ids_with_data + $this->limit;
            for ($limit=$this->limit; $limit<=$total; $limit=$limit+$this->limit)
            {
                $i++;
                $path_to_archive_directory = $this->TEMP_DIR . $i . '_working/';
                echo "\n[$path_to_archive_directory]";
                $this->temp_archive_builder = new \eol_schema\ContentArchiveBuilder(array('directory_path' => $path_to_archive_directory));
                self::csv_to_array($this->text_path["IRMNG_DWC_SP_PROFILE"], "extant_habitat_data", array(), $limit);
                
                self::combine_working_folders($i);
                $this->temp_archive_builder->finalize(TRUE);
                $this->temp_archive_builder = null;
                
                // remove temp subdir
                $dir = $this->TEMP_DIR . $i . '_working';
                recursive_rmdir($dir);
                debug("\n\n temporary subdirectory removed: " . $dir . "\n\n");
            }
        }
        else
        {
        ...
        print_r($this->debug_status);
        print_r($this->debug_status_count);
    }
    
    private function process_data_classification()
    {
        $taxa_ids_with_data = array();
        $taxa_ids_with_data = self::csv_to_array($this->text_path["IRMNG_DWC_SP_PROFILE"], "get_taxa_ids_with_data");
        echo "\n\n" . count($taxa_ids_with_data) . "\n";
        self::csv_to_array($this->text_path["IRMNG_DWC"], "classification", $taxa_ids_with_data);
        self::end_first_schema();
        return count($taxa_ids_with_data);
    }
    
    private function end_first_schema()
    {
        ...
        $p = new \eol_schema\Occurrence();
        $p->occurrenceID = "";
        $p->taxonID = "";
        $this->archive_builder->write_object_to_file($p);
        $this->archive_builder->finalize(TRUE);
        $this->archive_builder = null;
    }
    
    private function combine_working_folders($folders)
    {
        $occurrence_file          = $this->path_to_archive_directory . "occurrence.tab";
        $measurement_or_fact_file = $this->path_to_archive_directory . "measurement_or_fact.tab";
        echo "\n\n total working folders: [$folders]\n";
        for ($i=$folders; $i<=$folders; $i++)
        {
            $contents = file_get_contents($this->TEMP_DIR . $i . '_working/occurrence_working.tab', true);
            self::save_to_dump($contents, $occurrence_file);
            $contents = file_get_contents($this->TEMP_DIR . $i . '_working/measurement_or_fact_working.tab', true);
            self::save_to_dump($contents, $measurement_or_fact_file);
        }
    }
    
    private function csv_to_array($csv_file, $type, $taxa_ids_with_data = array(), $limit = false)
    {
        ...
        while(!feof($file))
        {
            ...
            else
            {
                ...
                if($limit)
                {
                    if($i < $limit-$this->limit) continue;
                    if($i >= $limit-$this->limit && $i < $limit) {}
                    if($i >= $limit) return;
                }
                ...
                if($type == "get_taxa_ids_with_data") $taxon_ids[$taxon_id] = 1; // for $this->resource_type == "data"
                elseif($type == "classification")
                {
                    if($this->resource_type == "data")
                    {
                        if(isset($taxa_ids_with_data[$taxon_id])) $this->create_instances_from_taxon_object($rec);
                    }
                    else $this->create_instances_from_taxon_object($rec);
                }
    ...
    private function create_instances_from_taxon_object($rec)
    {
        ...
        */
        ...
        if($this->resource_type == "data")
        {
            ...
            // "data" resource doesn't need synonyms, only the "classification" resource needs synonyms
            if($rec["TAXONOMICSTATUS"] == "synonym") return;
        }
        elseif($this->resource_type == "classification")
        {
            if($val = trim($rec["PARENTNAMEUSAGEID"]))
            {
                if($val != $taxon->taxonID) $taxon->parentNameUsageID = $val;
            }
        }
    ...
    private function add_string_types($rec, $label, $value, $mtype)
    {
        ...
        $this->temp_archive_builder->write_object_to_file($m);
    }

    private function add_occurrence($taxon_id, $catnum)
    {
        $occurrence_id = $taxon_id . '_' . $catnum;
        // if(isset($this->occurrence_ids[$occurrence_id])) return $this->occurrence_ids[$occurrence_id]; -- this causes to slowdown for million records
        $o = new \eol_schema\Occurrence();
        $o->occurrenceID = $occurrence_id;
        $o->taxonID = $taxon_id;
        $this->temp_archive_builder->write_object_to_file($o);
