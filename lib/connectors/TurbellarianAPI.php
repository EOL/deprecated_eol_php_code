...
class TurbellarianAPI
{
        ...
        if($taxa_arr["images"])
        {
            ...
            foreach($arr as $r)
            {
                ...
                */
                ...
                $mimeType   = Functions::get_mimetype($img);
    ...
    function prepare_distribution($html)
    {
        ...
        if(preg_match("/<th>reference<\/th>(.*?)<\/table>/ims", $html, $matches))
        {
            ...
            foreach($arr as $r)
            {
                ...
                if(trim($dist_ref) != "|nbsp; |nbsp;. |nbsp; |nbsp;") $unique_ref[$ref_href] = $dist_ref;
            }
        }
        ...
        if($dist)
        {
            ...
            {
                $ref_temp = "(Ref.: " . str_ireplace("|", "&", @$dist[$d]["ref_short"]) . ")";
                if($ref_temp != "(Ref.: &nbsp; &nbsp;)") $str .= "$d &nbsp;&nbsp; " . $ref_temp . "<br><br>";
            }
        }
        $refs = array();
    ...
    {
        $str = str_ireplace(array("\n", "\r", "\t", "\o", "\xOB", "\xA0", "\xAO", "\xB0", "\xa0", "\xaO", "0xC3", "0x20", "0x70", "0x6C", "\xc2", "\x1a"), "", $str);
        return $str;
    }
}
?>