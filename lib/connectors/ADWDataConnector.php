...
class ADWDataConnector
{
    ...
    public function process_line_data($line_data)
    {
        ...
        $this->add_boolean_types($line_data, $taxon->taxonID);
        $this->add_numeric_types($line_data, $taxon->taxonID);
        $this->add_string_types($line_data, $taxon->taxonID);
        $this->add_predators($line_data, $taxon->taxonID);
    }
    ...
    private function add_boolean_types($line_data, $taxon_id)
    {
    ...
    private function add_string_types($line_data, $taxon_id)
    {
    ...
    private function add_predators($line_data, $taxon_id)
    {
        if($v = trim($line_data[$this->column_indices['Known Predators']]))
        {
            ...
            foreach($values as $value)
            {
                ...
                $m->taxonID = $taxon_id;
                $m->associationType = "http://adw.org/hasPredator";
                $m->targetTaxonID = $related_taxon->taxonID;
                $this->archive_builder->write_object_to_file($m);
            }
        }
    }
