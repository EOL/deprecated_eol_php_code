...
class MoveEntryHandler
{
  ...
  function move_entry($args)
  {
    ...
    $tc_from = php_active_record\TaxonConcept::find($args['taxon_concept_id_from']);
    $tc_to = php_active_record\TaxonConcept::find($args['taxon_concept_id_to']);
    ...
    if($args['confirmed'] == 'confirmed' || $args['confirmed'] == 'force')
    {
        ...
        php_active_record\HierarchyEntry::move_to_concept_static($args['hierarchy_entry_id'], $args['taxon_concept_id_to'], $force_move_if_disallowed, $args['reindex']);
        ...
        if($args['reindex_solr'] == 'reindex_solr') // NOTE - this can ONLY be specified by the resque task, ATM.
        {
          ...
          php_active_record\SolrUpdateConceptHandler::update_concept($args['taxon_concept_id_from']);
          php_active_record\SolrUpdateConceptHandler::update_concept($args['taxon_concept_id_to']);
        }
    ...
    }else
    {
        ...
        $descendant_objects = php_active_record\TaxonConcept::count_descendants_objects($tc_from->id);
        $descendants = php_active_record\TaxonConcept::count_descendants($tc_from->id);
        echo "\n\nTaxonConcept1: $tc_from->id\n";
        echo "Descendant Objects:  $descendant_objects\n";
        echo "Descendant Concepts: $descendants\n";
        
        $descendant_objects = php_active_record\TaxonConcept::count_descendants_objects($tc_to->id);
        $descendants = php_active_record\TaxonConcept::count_descendants($tc_to->id);
