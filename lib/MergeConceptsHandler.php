...
class MergeConceptsHandler
{
  ...
  function merge_concepts($args)
  {
    ...
    if($args['confirmed'] == 'confirmed')
    {
        ...
        php_active_record\TaxonConcept::supercede_by_ids($args['id1'], $args['id2'], true);
        ...
        if($args['reindex'] == 'reindex') // NOTE - this can ONLY be specified by the resque task, ATM.
        {
          ...
          php_active_record\SolrUpdateConceptHandler::update_concept($args['id1']);
          php_active_record\SolrUpdateConceptHandler::update_concept($args['id2']);
        }

    }else
    {
        $descendant_objects = php_active_record\TaxonConcept::count_descendants_objects($args['id1']);
        $descendants = php_active_record\TaxonConcept::count_descendants($args['id1']);
        echo "\n\nTaxonConcept1: " . $args['id1'] . "\n";
        echo "Descendant Objects:  $descendant_objects\n";
        echo "Descendant Concepts: $descendants\n";
        
        $descendant_objects = php_active_record\TaxonConcept::count_descendants_objects($args['id2']);
        $descendants = php_active_record\TaxonConcept::count_descendants($args['id2']);
