...
class ContentManager
{
	...
	function grab_file($file, $type, $options = array())
	{
		...
		if($type==="image")
		{
			$result = $this->create_local_files_and_thumbnails($cache_file_path, $permanent_prefix, $options);
			if(is_array($result))
			{
				$create_image_result = $result; //image info(width, height, cropping info)
			}
		}
		...
		switch($type) {
			...
			case "partner":
				...
				{
					// Take the substring of the new file path to return via the webservice, so that /content/2009/05/19/23/85866.png -> 200905192385866
					// image_info : (width, height, cropping info)
				    return array('file_path' => self::cache_path2num($permanent_file_path), 'image_info' => $create_image_result); 
				}
	...
	function create_local_files_and_thumbnails($original_file, $prefix, $options = array())
	{
		...
		$res = array('width' => $width, 'height' => $height,'custom_crop' => $custom_crop);
		return $res;	
	}

	function keep_cropping_info($options)
	{
		$this->save_image_size_data($options['data_object_id'], $options['image_info']['width'], $options['image_info']['height'], $options['image_info']['custom_crop']);		
	}
...
//		if(isset($data_object_id) && isset($width) && isset($height))
//		{

//			$GLOBALS['mysqli_connection']->insert("INSERT IGNORE INTO image_sizes (data_object_id, width, height) VALUES ($data_object_id, $width, $height)");
//		}
	...
	function save_image_size_data($data_object_id, $width, $height, $crop_percentages=NULL)
	{
		if (isset($data_object_id)) {
			...
			$sql_attributes = "INSERT IGNORE INTO image_sizes (data_object_id, width, height";
			$sql_values = "VALUES ($data_object_id, $width, $height";
			...
			{   //also set the crop values at the same time
				...
				{ //the 4th %age (height) might be unset
					$sql_attributes .= ", crop_x_pct, crop_y_pct, crop_width_pct, crop_height_pct";
					$sql_values .= vsprintf(",%.2F,%.2F,%.2F,NULL", $crop_percentages);
				} else {
					$sql_attributes .= ", crop_x_pct, crop_y_pct, crop_width_pct, crop_height_pct";
					$sql_values .= vsprintf(",%.2F,%.2F,%.2F,%2F", $crop_percentages);
				}
			}
			$sql_attributes .= ")";
			$sql_values .= ")";
